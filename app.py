...
@st.cache_resource
def create_retrieval_qa(folder_path = r'Documents/'): 	
	# Might have to change the glob regex to look for different kinds of files
...
uploaded_file = st.file_uploader(
    "Upload a document", type=("pdf, xml, txt, docx")
)

qa = create_retrieval_qa() # Running it with default dataset
if uploaded_file:
	try:
		...
		qa = create_retrieval_qa(tmp_path)
		# Remove temporary file after processing
		os.remove(tmp_path)

	except Exception as e:
		st.write(f"Error processing email: {str(e)}")
